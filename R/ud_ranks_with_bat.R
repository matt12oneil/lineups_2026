...
steamer_multiple <- runif(1,min=0,max = 1)
depth_charts_multiple <- runif(1,min=0,max = 1)
bat_multiple <- runif(1,min=0,max=1)
multiples <- steamer_multiple + depth_charts_multiple + bat_multiple
steamer_multiple = steamer_multiple/multiples*3
depth_charts_multiple = depth_charts_multiple/multiples*3
bat_multiple = bat_multiple/multiples*3
steamer_multiple + depth_charts_multiple + bat_multiple
...
batx_batters <- read_csv(file = 'Inputs/batx_batters.csv') |>
  clean_names() |>
  mutate(full_name = stri_trans_general(str = name, 
                                        id = "Latin-ASCII")) |>
  mutate(ud_points = 3*x1b + 6*x2b + 8*x3b + 10*hr + 3*bb + 3*ibb + 3*hbp + 2*r + 2*rbi + 4*sb) |>
  ...
  mutate(proj_rank = rank(-ud_points), rand_rank = rank(-points)) |>
  arrange(desc(ud_points)) |>
  mutate(points = points*bat_multiple) |>
  rename(name = name_ascii)
...
bat_pitchers <- read_csv(file = 'Inputs/bat_pitchers.csv') |>
  clean_names() |>
  filter(ip >= 50) |>
  mutate(full_name = stri_trans_general(str = name, 
                                        id = "Latin-ASCII")) |>
...
proj <- batter_proj |>
  ...
  select(-player_id) |>
  mutate(value = round(adp_rank/((adp_rank + 3*points_rank)/4),1)) |>
  filter(!is.na(value)) |>
  mutate(value_pct = ((adp_rank + 3*points_rank)/4)) |>
  ...
  mutate(value = case_when(value <= .7 ~ 1, 
                           ...
                           value > 2.5 ~ 6,
                           .default = 3)) |>
  select(name, team, position, value, value_pct, adp_rank, points_rank, adp, points) |>
  group_by(position) |>
  ...
  arrange(adp) |>
  select(name, team, position, vorp, value, value_pct, adp_rank, points_rank, adp, points) |>
  mutate(vorp = round(rank(-vorp)), adp_rank = round(adp_rank), points_rank = round(points_rank))
